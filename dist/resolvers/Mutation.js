"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var bcrypt = require("bcrypt");
var jwt = require("jsonwebtoken");
var utils_1 = require("../utils");
var clientSecret = 'qwe123gdfs324';
exports.Mutation = {
    createUser: function (root, _a, _b) {
        var email = _a.email, name = _a.name, password = _a.password;
        var prisma = _b.prisma;
        return __awaiter(this, void 0, void 0, function () {
            var hashedPassword, user;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        if (password.length < 8) {
                            throw new Error('Password must be at lease 8 characters long');
                        }
                        return [4, bcrypt.hash(password, 10)];
                    case 1:
                        hashedPassword = _c.sent();
                        return [4, prisma.mutation.createUser({
                                data: {
                                    email: email,
                                    name: name,
                                    password: hashedPassword,
                                },
                            })];
                    case 2:
                        user = _c.sent();
                        return [2, {
                                user: user,
                                token: jwt.sign({ user: user }, clientSecret),
                            }];
                }
            });
        });
    },
    login: function (root, _a, _b) {
        var email = _a.email, password = _a.password;
        var prisma = _b.prisma;
        return __awaiter(this, void 0, void 0, function () {
            var user, match;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0: return [4, prisma.query.user({ where: { email: email } })];
                    case 1:
                        user = _c.sent();
                        if (!user) {
                            throw new Error('there is no user with this mail address');
                        }
                        return [4, bcrypt.compare(password, user.password)];
                    case 2:
                        match = _c.sent();
                        if (!match) {
                            throw new Error('wrong password');
                        }
                        return [2, {
                                user: user,
                                token: jwt.sign({ user: user }, clientSecret),
                            }];
                }
            });
        });
    },
    updateUser: function (root, _a, _b) {
        var name = _a.name;
        var prisma = _b.prisma, request = _b.request;
        return __awaiter(this, void 0, void 0, function () {
            var userId, user;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        userId = utils_1.default(request);
                        return [4, prisma.mutation.updateUser({
                                data: { name: name },
                                where: { id: userId },
                            })];
                    case 1:
                        user = _c.sent();
                        return [2, user];
                }
            });
        });
    },
    createPost: function (root, _a, _b) {
        var title = _a.title;
        var prisma = _b.prisma, request = _b.request;
        return __awaiter(this, void 0, void 0, function () {
            var userId, post;
            return __generator(this, function (_c) {
                switch (_c.label) {
                    case 0:
                        userId = utils_1.default(request);
                        if (!userId) {
                            throw new Error('authorization is required');
                        }
                        return [4, prisma.mutation.createPost({
                                data: {
                                    title: title,
                                },
                            })];
                    case 1:
                        post = _c.sent();
                        return [2, post];
                }
            });
        });
    },
};
